...
// const ProductDetails = async ({ params: { slug }, searchParams: { size } }: { params: { slug: string }; searchParams: { size?: string } }) => {
//   const product = await getProductBySlug(slug);
//   if (!product) notFound();
//   const cart = await getMyCart();
//   const session = await auth();
...
//                 </p>
//                 <h1 className="h3-bold max-w-md">{product.name}</h1>
//                 <Rating value={Number(product.rating)} caption={`${product.numReviews} reviews`} />

//                 <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
//                   <div className="flex gap-3">
//                     <ProductPrice value={Number(product.price)} className="p-bold-20 rounded-full border-2 bg-[#41B3A2] px-3 py-1 text-white" />
//                   </div>
//                 </div>
//                 <SizeSelector sizes={product.sizes} currentSize={size} productSlug={slug} />
//               </div>
//               <div className="mx-auto w-full min-w-52 sm:mr-0 sm:w-auto">
//                 <Card>
//                   <CardContent className="p-4">
//                     <div className="mb-2 flex justify-between">
//                       <div>Price</div>
...
//                     {product.stock !== 0 && (
//                       <div className="flex-center">
//                         <AddToCart cart={cart} item={{ productId: product.id, name: product.name, slug: product.slug, price: round2(product.price), qty: 1, image: product.images![0], size }} />
//                       </div>
//                     )}
//                   </CardContent>
//                 </Card>
...
//         <h2 className="h2-bold mb-5">Customer Reviews</h2>
//         {/* eslint-disable-next-line @typescript-eslint/no-non-null-asserted-optional-chain */}
//         <ReviewList productId={product.id} productSlug={product.slug} userId={session?.user.id!} />
//       </section>
//     </div>
//   );
// };
