...
export async function deleteImageFromUploadthing(url: string) {
  try {
    // Extract the filename or ID from the URL (after the last '/')
    const fileId = url.substring(url.lastIndexOf('/') + 1);

    // Initialize the UploadThing API
    const utapi = new UTApi();

    // Use the UTApi to delete the file by its ID
    await utapi.deleteFiles(fileId);

    return { success: true, message: 'File deleted successfully' };
  } catch (error: any) {
    console.error('Error deleting file:', error);
    return { success: false, message: `Error deleting file: ${error.message || error}` };
  }
}
...
const reverseGeocode = async (lat: number, lng: number) => {
  try {
    ...
    const { road, city, country, postcode } = response.data.address || {};
    return {
      streetAddress: road || '',
      city: city || '',
      country: country || '',
