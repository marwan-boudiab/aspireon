...
export async function getReviews({ productId, limit = PAGE_SIZE, page }: { productId: string; limit?: number; page: number }) {
  ...
  const dataCount = await db.select({ count: count() }).from(reviews).where(eq(reviews.productId, productId));
  return {
    data,
...
export const getUserReviewByProductId = async ({ productId }: { productId: string }) => {
  const session = await auth();
  if (!session) throw new Error('User is not authenticated');
