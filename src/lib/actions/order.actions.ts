...
export async function getMyOrders({ limit = PAGE_SIZE, page }: { limit?: number; page: number }) {
  ...
  const dataCount = await db.select({ count: count() }).from(orders).where(eq(orders.userId, session.user.id!));
...
export async function getOrderSummary() {
  ...
  const salesData = await db
    ...
    .from(orders)
    .where(eq(orders.isPaid, true)) // Adding the condition where isPaid is true
    .groupBy(sql`1`);
  // .groupBy(sql`to_char(${orders.createdAt}, 'MM/YY')`);  // Grouping by the formatted date
...
export async function approvePayPalOrder(orderId: string, data: { orderID: string }) {
  try {
    ...
    if (!captureData || captureData.id !== order.paymentResult?.id || captureData.status !== 'COMPLETED') throw new Error('Error in paypal payment');
...
export const updateOrderToPaid = async ({ orderId, paymentResult }: { orderId: string; paymentResult?: PaymentResult }) => {
  const order = await db.query.orders.findFirst({
    columns: { isPaid: true },
    where: eq(orders.id, orderId),
...
export async function cancelOrder(orderId: string) {
  try {
    ...
    // CHECK IF ORDER EXISTS AND IS WITHIN 24 HOURS
    const order = await db.query.orders.findFirst({ where: eq(orders.id, orderId) });
    if (!order) return { success: false, message: 'Order not found' };
    if (new Date(order.createdAt) < oneDayAgo) return { success: false, message: 'Orders can only be canceled within 24 hours of creation' };
    if (order.isPaid) return { success: false, message: 'Paid orders cannot be canceled' };
    if (order.isDelivered) return { success: false, message: 'Delivered orders cannot be canceled' };
    // DELETE ORDER
    await db.delete(orders).where(eq(orders.id, orderId));
    // REVALIDATE USER ORDERS LIST
    revalidatePath('/user/orders');
    // RETURN SUCCESS MESSAGE
    return { success: true, message: 'The order has been canceled successfully' };
  } catch (error) {
    console.error('Error canceling order:', error);
    // RETURN ERROR MESSAGE
    return { success: false, message: 'Error while canceling order' };
  }
}
